...
public class Dice : MonoBehaviour 
{
	...
	void Update () 
    {
        if (isRolling == true && menuscripts.menuinstance.gameisonline == false)
        {
            if (rb.IsSleeping())
            {
                isRolling = false;
        ...
        if (isRolling == true && menuscripts.menuinstance.gameisonline == true)
        {
            if (rb.IsSleeping())
            {
                isRolling = false;
                calculateValue();
                DiceManager.Diceinstance.Onlinecountalldievalues();
            }
        }
    ...
    // This function is used to get the value from the dice to roll 
    void calculateValue()
    {
